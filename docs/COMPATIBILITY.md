...
- **live-coedit (AI Co-Editor):** two interactions, both observed live. First, remote collaborators' edits arrive as editor transactions, so on a checked-out note they are blocked like any other edit — pull first, then collaborate. Second, a pull writes the note from outside the editor, and AI Co-Editor correctly flags that as an external change: after pulling a note that AI Co-Editor is watching, you may see a review proposal containing your own reMarkable edits. Accepting it is harmless (it is the text you already have); rejecting it reverts the pull, so accept or dismiss rather than reject.
